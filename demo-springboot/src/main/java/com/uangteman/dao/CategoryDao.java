...
public class CategoryDao {
	...
	public int update(Category category) {
		return this.template.update("update tcategory set name=? where " + "id=?",
				category.getName(),
				category.getId());
	}
	...
	public Category findById(int id){
		return this.template.queryForObject("select id,name from tcategory "
				+ "where id=?", new Object[]{id}, new RowMapper<Category>() {
					@Override
					public Category mapRow(ResultSet rs, int index) 
							throws SQLException {
						Category category = new Category();
